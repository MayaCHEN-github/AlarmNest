...
/* AlarmService.kt
 * 用于处理闹钟相关的服务，包括创建通知渠道、启动闹钟音乐、定时发送通知等功能。
* */
...
class AlarmService : Service() {
    companion object {
        ...
        private const val NOTIFICATION_INTERVAL = 2000L // 2秒发送一次通知
    }

    private var wakeLock: PowerManager.WakeLock? = null // 用于保持CPU处于唤醒状态, 防止手机进入休眠状态!
        ...
        try {
            ...
            mediaPlayer = MediaPlayer().apply {
                try {
                    ...
                    release()
                    mediaPlayer = null
                    android.os.Handler(mainLooper).postDelayed({ // 延迟1秒后重新启动闹钟音乐
                        startAlarmSound()
                    }, 1000)
                }
            }
    ...
    private fun createNotification(): Notification {
        try { // 尽可能全部从Intent中读取过来……
            val alarmId = currentIntent?.getLongExtra("alarm_id", -1) ?: -1 // 闹钟ID
            val isSubAlarm = currentIntent?.getBooleanExtra("is_sub_alarm", false) ?: false // 是否是子闹钟
            val alarmLabel = currentIntent?.getStringExtra("alarm_label") ?: "闹钟提醒" // 闹钟标签
            val alarmHour = currentIntent?.getIntExtra("alarm_hour", 0) ?: 0 // 闹钟小时
            val alarmMinute = currentIntent?.getIntExtra("alarm_minute", 0) ?: 0 // 闹钟分钟
            val alarmRepeatType = currentIntent?.getStringExtra("alarm_repeat_type") ?: "ONCE" // 闹钟重复类型
            val alarmCustomDays = currentIntent?.getStringExtra("alarm_custom_days") ?: "" // 闹钟自定义天数
            val alarmDismissType = currentIntent?.getStringExtra("alarm_dismiss_type") ?: "NO_ALARM" // 闹钟关闭类型
            val alarmTriggerType = currentIntent?.getStringExtra("alarm_trigger_type") ?: "TIME" // 闹钟触发类型
            val alarmIsEnabled = currentIntent?.getBooleanExtra("alarm_is_enabled", true) ?: true // 闹钟是否启用
            ...
            return NotificationCompat.Builder(this, CHANNEL_ID) // 创建通知
                .setSmallIcon(android.R.drawable.ic_dialog_alert)
                .setContentTitle(if (isSubAlarm) "子闹钟: $alarmLabel" else alarmLabel) // 如果是子闹钟，则显示子闹钟标签
